...
import { ExternalLink, GlobeIcon, UsersIcon, TrendingUpIcon, ActivityIcon, CheckCircleIcon } from '../Icons';
...
export const ProductDetail = ({ entry, onClose }: { entry: Entry, onClose: () => void }) => {
    ...
    // Helper to map icons to metric labels loosely
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 lg:p-4 overflow-hidden">
            ...
            <div className="relative w-full h-full lg:max-w-[1400px] lg:h-[90vh] bg-[#0a0a0a] text-white shadow-2xl flex flex-col overflow-hidden animate-slide-up lg:rounded-[4px] border border-white/10">
                ...
                <div className="h-16 border-b border-white/10 bg-[#0a0a0a] flex items-center justify-between px-6 shrink-0 relative z-50">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-white text-black flex items-center justify-center font-serif font-bold text-lg rounded-[2px]">
                    ...
                    <div className="flex items-center gap-4">
                        {entry.link && (
                             <a 
                                ...
                                target="_blank" 
                                rel="noreferrer"
                                className="hidden lg:flex items-center gap-2 px-4 py-2 bg-white text-black font-mono text-xs font-bold uppercase hover:bg-accent hover:text-white transition-colors"
                             >
                        ...
                        <button 
                            onClick={onClose} 
                            className="w-8 h-8 flex items-center justify-center border border-white/20 text-white/50 hover:text-white hover:border-white transition-colors"
                        >
                            ✕
                        </button>
                    </div>
                ...
                {/* --- MAIN LAYOUT: Split Screen on Desktop (LG+), Stack on Mobile/Tablet --- */}
                <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
                    
                    {/* LEFT PANEL: The Narrative (Scrollable) */}
                    {/* On mobile, this is Order 1 (Top). On Desktop, Order 1 (Left). */}
                    <div className="flex-1 overflow-y-auto technical-scrollbar bg-[#0a0a0a] relative order-1 lg:order-1 flex flex-col">
                        ...
                        <div className="relative px-6 py-12 lg:p-16 border-b border-white/10 overflow-hidden shrink-0">
                            ...
                            <div className="relative z-10 max-w-4xl mx-auto w-full">
                                <div className="inline-block px-3 py-1 mb-6 border border-accent text-accent font-mono text-[10px] font-bold uppercase tracking-[0.2em]">
                                    Market Ready /// L3
                                </div>
                                <h1 className="font-serif text-5xl lg:text-7xl font-bold leading-[0.9] mb-8 tracking-tight">
                                    {entry.title}.
                                </h1>
                                <p className="font-sans text-xl text-white/70 leading-relaxed border-l-2 border-white/20 pl-6">
                                    {entry.description}
                                </p>
                            </div>
                        </div>
                        ...
                        {/* Mobile Action Footer (Visible only on small screens, below content) */}
                        <div className="lg:hidden p-4 border-t border-white/10 bg-[#0a0a0a]/90 backdrop-blur-md shrink-0">
                            {entry.link && (
                                <a 
                                    ...
                                    target="_blank" 
                                    rel="noreferrer"
                                    className="flex items-center justify-center w-full py-4 bg-accent text-white font-mono font-bold uppercase tracking-widest text-sm shadow-[0_0_20px_rgba(255,79,0,0.4)]"
                                >
                                    Launch Application
                                </a>
                            )}
                    ...
                    {/* RIGHT PANEL: Business Intelligence (Collapsible on Mobile) */}
                    {/* On mobile, this is Order 2 (Bottom). On Desktop, Order 2 (Right). */}
                    <div className={`
                        w-full lg:w-[450px] xl:w-[500px] bg-[#0f0f0f] border-l border-white/10 
                    ...
                    `}>
                        ...
                        <div 
                            className="lg:hidden flex items-center justify-between p-4 bg-[#111] border-b border-white/10 cursor-pointer shrink-0"
                            onClick={() => setIsMobileMetaExpanded(!isMobileMetaExpanded)}
                        >
                            <div className="flex items-center gap-2">
                                <ActivityIcon className="w-4 h-4 text-accent" />
                                <span className="font-mono text-[10px] uppercase font-bold tracking-widest text-white">Live Metrics</span>
                            </div>
                        ...
                        <div className={`flex-1 overflow-y-auto technical-scrollbar transition-opacity duration-300 ${isMobileMetaExpanded ? 'opacity-100' : 'opacity-0 lg:opacity-100'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-px bg-white/10 border-b border-white/10">
                                {entry.metrics?.map((m, i) => (
                                    <div key={i} className="bg-[#111] p-6 group hover:bg-[#161616] transition-colors relative overflow-hidden">
                                        ...
                                        <div className="font-sans text-3xl font-bold text-white group-hover:scale-105 transition-transform origin-left">
                                            {m.value}
                                        </div>
                                ...
                                {(!entry.metrics || entry.metrics.length === 0) && (
                                    <div className="col-span-2 p-8 text-center text-white/30 font-mono text-xs italic">
                                        Connecting to telemetry stream...
                                    </div>
                                )}
                            </div>

                            {/* 2. Timeline / Release History */}
                            <div className="p-8 flex-1">
                                <h3 className="font-mono text-xs text-white/40 uppercase tracking-widest mb-8 flex items-center gap-2">
                                    <ActivityIcon className="w-3 h-3" /> Release Log
                                </h3>
                                ...
                                <div className="relative border-l border-white/10 ml-3 space-y-8 pb-8">
                                    {entry.updates?.slice().reverse().map((u, i) => (
                                        <div key={i} className="relative pl-8 group">
                                            {/* Node */}
                                            <div className={`
                                                absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full border border-[#0f0f0f] shadow-[0_0_0_2px_#333] transition-colors
                                                ${i === 0 ? 'bg-accent shadow-[0_0_0_2px_#FF4F00] animate-pulse' : 'bg-[#333] group-hover:bg-white'}
                                            `} />
                                            
                                            {/* Content */}
                                            <div className="flex flex-col">
                                                <div className="flex items-baseline justify-between mb-1">
                                                    <span className={`font-mono text-xs font-bold ${i === 0 ? 'text-white' : 'text-white/60'}`}>
                                                        {u.version || 'UPDATE'}
                                                    </span>
                                                ...
                                                {u.type === 'release' && (
                                                    <div className="mt-2 inline-flex items-center gap-1 text-[9px] text-accent uppercase font-bold tracking-wider">
                                                        <CheckCircleIcon className="w-3 h-3" /> Milestone Reached
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                            ...
                            {/* 3. Tech Stack / Tags (Footer of Right Panel) */}
                            <div className="p-8 border-t border-white/10 bg-[#0a0a0a]">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {entry.tags.map(tag => (
                                        <span key={tag} className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs text-white/60 hover:text-white hover:border-white/40 transition-colors cursor-default">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
