...
import { CoffeeStain, TapeStrip, DoodleStar, DoodleArrow, PaperHoles } from '../ui/HandwrittenEffects';
...
export const LabNoteDetail = ({ entry, onClose }: { entry: Entry, onClose: () => void }) => {
    // We treat the pages as a simple queue. pages[0] is always the top card.
    ...
    const handleFlip = (e: React.MouseEvent) => {
        ...
        // Animation timing:
        // 1. Trigger "fly out" animation via state/class
        // 2. Wait for animation to finish
        // 3. Rotate array (move top to bottom)
        ...
        setTimeout(() => {
            ...
            setIsFlipping(false);
        }, 250); // Speeded up from 400ms to 250ms for snappier feel
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-hidden">
            {/* Backdrop */}
            <div 
                className="absolute inset-0 bg-ink/70 backdrop-blur-sm transition-opacity duration-500 animate-fade-in" 
                onClick={onClose} 
            />
            
            <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
                
                {/* Close Button - Positioned safely for mobile */}
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 md:top-8 md:right-12 z-[100] pointer-events-auto text-white font-hand text-2xl md:text-3xl hover:scale-110 transition-transform drop-shadow-md"
                >
                    (close)
                </button>
                ...
                {/* The Stack Container */}
                <div 
                    className="relative w-[90vw] md:w-[600px] h-[70vh] md:h-[800px] max-h-[85dvh] perspective-1000 pointer-events-auto"
                    onClick={handleFlip}
                >
                    {/* Render cards in reverse order so the first in array is visually on top (z-index) */}
                    {pages.map((page, index) => {
                        // We only render the top 3 cards for performance and visual clarity
                        if (index > 2) return null;

                        const isTop = index === 0;
                        
                        // Dynamic styles for the stack effect
                        let transform = '';
                        let opacity = 1;
                        let zIndex = 50 - index;
                        ...
                        if (isTop) {
                            ...
                            transform = isFlipping 
                                ? 'translate(120%, 10%) rotate(15deg)' // Fly out to right
                                : 'translate(0, 0) rotate(0deg)';      // Resting state
                        } else {
                            // Background Cards (Static stack look)
                            // Card 2: slightly rotated left, Card 3: slightly rotated right
                        ...
                        return (
                            <div
                                key={page.id}
                                className={`
                                    absolute inset-0 bg-[#fdfbf7] shadow-paper rounded-sm 
                                    flex flex-col overflow-hidden border border-gray-300
                            ...
                            >
                                ...
                                <div className="relative z-10 p-6 md:p-12 h-full flex flex-col">
                                    ...
                                    {/* Main Title */}
                                    {page.type === 'update' ? (
                                        <h2 className="font-marker text-3xl md:text-5xl mb-6 text-ink leading-[0.9]">
                                            Update: {page.date}
                                        </h2>
                                    ) : (
                                        <h2 className="font-marker text-3xl md:text-5xl mb-6 text-ink leading-[0.9]">
                                    ...
                                    <div className="flex-1 overflow-y-auto relative pr-2">
                                        ...
                                        {/* Footer Annotation */}
                                        <div className="mt-8 mb-4 font-hand text-2xl text-gray-300 transform -rotate-2 select-none">
                                            (end of note)
                                        </div>

                                        {/* Random Doodle on First Page */}
                                        {page.type === 'main' && (
                    ...
                    {/* Interaction Hint (Only shows if multiple pages) */}
                    {pages.length > 1 && (
