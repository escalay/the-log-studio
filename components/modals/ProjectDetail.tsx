...
import { BookIcon, TerminalIcon, BoxIcon, BeakerIcon } from '../Icons';
...
interface MockFile {
    ...
    content?: string; // If undefined, it's a folder or special handling
    language?: string;
}
...
export const ProjectDetail = ({ entry, onClose }: { entry: Entry, onClose: () => void }) => {
    ...
    // Mock Files System based on entry data
    const files: MockFile[] = useMemo(() => [
        { 
            name: 'README.md', 
            type: 'file', 
            icon: <BookIcon className="w-3 h-3 text-accent" />,
            language: 'markdown'
        },
        { 
            name: 'package.json', 
            type: 'file', 
            icon: <TerminalIcon className="w-3 h-3 text-subtle" />,
            language: 'json',
            content: JSON.stringify({
                name: entry.slug,
                version: entry.updates?.[entry.updates.length - 1]?.version || '0.1.0',
                ...
                dependencies: Object.fromEntries(entry.tags.map(t => [t, "latest"])),
                license: "MIT"
            }, null, 2)
        },
        { 
            name: 'src', 
            type: 'folder', 
            icon: <BoxIcon className="w-3 h-3 text-subtle" /> 
        },
        { 
            name: 'src/index.ts', 
            type: 'file', 
            icon: <BeakerIcon className="w-3 h-3 text-subtle" />,
            language: 'typescript',
            content: `import { ${entry.title.replace(/\s/g, '')} } from './core';\n\n// Initialize the application\nconst app = new ${entry.title.replace(/\s/g, '')}({\n  env: process.env.NODE_ENV,\n  debug: true\n});\n\napp.start().then(() => {\n  console.log("System Online");\n});`
        },
        { 
            name: 'tests', 
            type: 'folder', 
            icon: <div className="w-3 h-3 border border-ink rounded-[1px]" /> 
        },
    ], [entry]);
    ...
    const handleFileClick = (file: MockFile) => {
        ...
        if (window.innerWidth < 1024) {
            setActiveTab('overview'); // Switch back to view on mobile/tablet
        }
    };
    ...
    const renderContent = () => {
            ...
            return (
                <div className="prose max-w-none font-sans animate-fade-in">
                    <div className="flex justify-between items-start border-b border-ink pb-8 mb-8">
                        ...
                        {entry.link && (
                            <a href={entry.link} target="_blank" rel="noreferrer" className="hidden lg:block bg-ink text-white px-6 py-2 font-mono text-xs font-bold hover:bg-accent hover:border-ink border border-transparent transition-all shadow-hard-sm">
                                VISIT DEPLOYMENT
                            </a>
                        )}
                    </div>
        ...
        return (
            <div className="font-mono text-sm animate-fade-in">
                <div className="mb-4 text-subtle text-xs uppercase tracking-widest border-b border-ink/10 pb-2">
            ...
            <div className="relative w-full max-w-6xl bg-white shadow-hard-xl pointer-events-auto animate-slide-up border-x border-y lg:border border-ink flex flex-col h-full lg:h-[85vh]">
                ...
                <div className="bg-level-0 border-b border-ink p-2 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-4 px-2">
                        ...
                        <div className="hidden lg:block font-mono text-xs bg-white border border-ink px-4 py-1 rounded-sm shadow-sm select-none">
                            ~/{entry.slug}/{selectedFile}
                        </div>
                    </div>
                    <button onClick={onClose} className="lg:hidden px-3 py-1 font-mono text-xs font-bold bg-ink text-white">CLOSE</button>
                    <div className="hidden lg:block text-[10px] font-mono text-subtle uppercase">
                        ReadOnly Mode
                    </div>
                </div>
                ...
                <div className="lg:hidden flex border-b border-ink bg-white shrink-0">
                    {(['overview', 'files', 'commits'] as const).map(tab => (
                        <button
                            ...
                            className={`flex-1 py-3 font-mono text-xs uppercase font-bold transition-colors ${activeTab === tab ? 'bg-ink text-white' : 'text-ink hover:bg-level-0'}`}
                        >
                            {tab}
                        </button>
                    ))}
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="hidden lg:flex w-72 bg-level-0 border-r border-ink flex-col overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 border-b border-ink">
                            <div className="font-mono text-[10px] uppercase text-subtle mb-4 font-bold tracking-wider flex items-center gap-2">
                                <BoxIcon className="w-3 h-3" /> Explorer
                            </div>
                            <ul className="space-y-1 font-mono text-xs">
                                {files.map((f, i) => (
                                    <li 
                                        ...
                                        className={`
                                            flex items-center gap-2 py-1.5 px-2 rounded-sm cursor-pointer transition-colors select-none
                                            ${selectedFile === f.name ? 'bg-white border border-ink shadow-sm' : 'hover:bg-white/50 border border-transparent'}
                                            ${f.type === 'folder' ? 'pl-2' : f.name.includes('/') ? 'ml-4' : ''}
                                        `}
                                    >
                                        {f.icon}
                        ...
                        <div className="h-1/2 overflow-y-auto p-4 bg-white/50">
                            <div className="font-mono text-[10px] uppercase text-subtle mb-4 font-bold tracking-wider flex items-center gap-2">
                                <TerminalIcon className="w-3 h-3" /> git log --oneline
                            </div>
                            <div className="relative border-l border-ink/20 ml-1.5 pl-4 space-y-4">
                                {entry.updates?.slice().reverse().map((u, i) => (
                                    <div 
                                        ...
                                        onClick={() => setExpandedCommit(expandedCommit === i ? null : i)}
                                    >
                                        ...
                                        <div className="font-bold leading-tight group-hover:text-accent transition-colors">
                                            {u.content}
                                        </div>
                                        ...
                                        {expandedCommit === i && (
                                            <div className="mt-2 p-2 bg-level-0 border border-ink animate-fade-in text-subtle">
                                                <div className="flex justify-between mb-1">
                                                    <span>Hash:</span>
                                                    <span className="font-bold">{Math.random().toString(16).substr(2, 7)}</span>
                                                </div>
                                                <div className="text-green-600">+ {Math.floor(Math.random() * 50)} additions</div>
                                                <div className="text-red-600">- {Math.floor(Math.random() * 20)} deletions</div>
                                            </div>
                                        )}
                                    </div>
                                ))}
                    ...
                    {/* Main Content (Desktop & Mobile/Tablet Overview Tab) */}
                    <div className={`flex-1 bg-white p-6 lg:p-12 overflow-y-auto ${activeTab === 'overview' ? 'block' : 'hidden lg:block'}`}>
                        {renderContent()}
                    </div>

                    {/* Mobile/Tablet Only: Files View */}
                         ...
                         <ul className="space-y-2 font-mono text-sm">
                            {files.map((f, i) => (
                                <li 
                                    ...
                                    onClick={() => handleFileClick(f)}
                                    className={`
                                        flex items-center gap-3 py-3 px-3 bg-white border border-ink shadow-sm active:translate-y-0.5 active:shadow-none
                                        ${selectedFile === f.name ? 'ring-2 ring-accent' : ''}
                                    `}
                                >
                                    {f.icon}
                    ...
                    {/* Mobile/Tablet Only: Commits View */}
