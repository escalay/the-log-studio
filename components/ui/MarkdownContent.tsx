    ...
    <>
        {content.split('\n').map((line, i) => {
            ...
            if (trimmed.startsWith('# ')) 
                return <h2 key={i} className={`${handwritten ? 'font-marker text-4xl mt-6 mb-2 text-ink/90 -rotate-1' : `font-serif text-4xl mt-8 mb-4 border-b-2 ${inverted ? 'border-white/20 text-white' : 'border-accent/20 text-ink'} pb-2 inline-block`}`}>{trimmed.replace('# ', '')}</h2>;
            
            if (trimmed.startsWith('## ')) 
                return <h3 key={i} className={`${handwritten ? 'font-hand text-2xl font-bold mt-6 mb-2 text-subtle uppercase' : `font-mono text-lg font-bold mt-8 mb-4 uppercase ${inverted ? 'text-white/60' : 'text-subtle'} tracking-wider`}`}>{trimmed.replace('## ', '')}</h3>;
...
// Helper to parse inline bold/strike/links within a line
