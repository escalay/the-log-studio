...
export const ExperimentDetail = ({ entry, onClose }: { entry: Entry; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 lg:p-6 xl:p-12 overflow-hidden">
      <div className="absolute inset-0 bg-ink/90 backdrop-blur-sm animate-fade-in cursor-pointer" onClick={onClose} />
      ...
      <div className="relative w-full h-full lg:max-w-7xl lg:h-[90vh] bg-paper shadow-2xl flex flex-col animate-slide-up border border-ink overflow-hidden lg:rounded-[2px]">
        ...
        <div className="h-14 lg:h-16 flex items-center justify-between border-b-2 border-ink bg-level-0 shrink-0 z-50 select-none">
          ...
          <div className="flex items-center h-full">
            ...
            <button onClick={onClose} className="h-full px-6 lg:px-8 bg-ink text-white hover:bg-accent hover:text-white transition-colors flex items-center justify-center gap-2 border-l-2 border-ink group relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-level-0 relative">
          ...
          <div className={`order-2 lg:order-1 w-full lg:w-80 xl:w-96 shrink-0 bg-[#1a1a1a] text-gray-300 border-t-2 lg:border-t-0 lg:border-r-2 border-ink flex flex-col overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] ${isMobileMetaExpanded ? 'h-[60vh]' : 'h-14'} lg:h-full`}>
            ...
            <div className={`flex-1 overflow-y-auto technical-scrollbar p-6 lg:p-8 space-y-8 transition-opacity duration-300 ${isMobileMetaExpanded ? 'opacity-100' : 'opacity-0 lg:opacity-100'}`}>
              <div className="bg-white/5 border border-white/10 p-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-1"><div className="flex gap-1"><div className="w-1 h-1 bg-white/20 rounded-full" /><div className="w-1 h-1 bg-white/20 rounded-full" /></div></div>
              ...
              <div>
                <h4 className="font-mono text-[9px] uppercase text-white/40 tracking-widest mb-3 font-bold border-l-2 border-accent pl-2">Performance Data</h4>
              ...
              <div>
                <h4 className="font-mono text-[9px] uppercase text-white/40 tracking-widest mb-3 font-bold border-l-2 border-accent pl-2">Activity Log</h4>
                  ...
                  {entry.updates?.slice().reverse().map((u, i) => (
                    <div key={i} className="relative pl-4 border-l border-white/10 hover:border-accent transition-colors pb-1">
                      ...
                      <div className="flex justify-between items-baseline mb-1 opacity-50">
                        <span>{u.date}</span>
                        <span className={`text-[8px] px-1 rounded-[1px] text-black font-bold uppercase ${u.type === 'fix' ? 'bg-red-400' : 'bg-green-400'}`}>{u.type}</span>
                      </div>
                      <div className="text-gray-300 leading-tight">{u.content}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* CONTENT PANEL */}
          <div className="order-1 lg:order-2 flex-1 bg-white overflow-y-auto technical-scrollbar relative h-full flex flex-col">
            <div className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-ink/10 px-6 lg:px-12 py-4 flex justify-between items-center shadow-sm">
              <div className="font-mono text-[10px] uppercase tracking-widest text-subtle">Document Viewer <span className="text-accent">///</span> Read-Only</div>
              <div className="hidden lg:block w-24 h-1 bg-ink/5 relative overflow-hidden"><div className="absolute inset-0 bg-ink/20 animate-slide-up" /></div>
            </div>

            <div className="relative z-10 p-6 lg:p-12 xl:p-16 max-w-4xl mx-auto flex-1">
              <header className="mb-10 lg:mb-16">
                <div className="inline-flex items-center gap-2 border border-accent text-accent font-mono text-[9px] font-bold px-2 py-1 mb-6 shadow-[2px_2px_0_0_#FF4F00] uppercase tracking-widest">Experiment Report</div>
                <h2 className="font-serif text-4xl lg:text-7xl font-bold leading-[0.9] mb-8 text-ink tracking-tight">{entry.title}</h2>
                <div className="bg-[#F8F8F8] p-6 border-l-4 border-ink relative">
                  <p className="font-mono text-xs lg:text-sm text-ink/80 leading-relaxed max-w-2xl">{entry.description}</p>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-end gap-6 opacity-60">
                <div>
                  <div className="w-48 h-12 border-b border-ink mb-2 relative">
                    <div className="absolute bottom-2 left-4 font-hand text-3xl text-blue-900 -rotate-6 mix-blend-multiply opacity-80">Approved</div>
                  </div>
